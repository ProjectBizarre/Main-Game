...
class MacroManager {
    constructor(typeChecker) {
        ...
        for (const [name, macro] of Object.entries(identifierMacros_1.IDENTIFIER_MACROS)) {
            const symbol = getGlobalSymbolByNameOrThrow(typeChecker, name, byots_1.default.SymbolFlags.Variable);
            this.identifierMacros.set(symbol, macro);
        }
        for (const [name, macro] of Object.entries(callMacros_1.CALL_MACROS)) {
            const symbol = getGlobalSymbolByNameOrThrow(typeChecker, name, byots_1.default.SymbolFlags.Function);
            this.callMacros.set(symbol, macro);
        }
        for (const [className, macro] of Object.entries(constructorMacros_1.CONSTRUCTOR_MACROS)) {
            const symbol = getGlobalSymbolByNameOrThrow(typeChecker, className, byots_1.default.SymbolFlags.Interface);
            const interfaceDec = getFirstDeclarationOrThrow(symbol, byots_1.default.isInterfaceDeclaration);
            ...
            for (const declaration of (_a = symbol.declarations) !== null && _a !== void 0 ? _a : []) {
                if (byots_1.default.isInterfaceDeclaration(declaration)) {
                    for (const member of declaration.members) {
                        if (byots_1.default.isMethodSignature(member) && byots_1.default.isIdentifier(member.name)) {
        ...
        for (const symbolName of Object.values(exports.SYMBOL_NAMES)) {
            const symbol = typeChecker.resolveName(symbolName, undefined, byots_1.default.SymbolFlags.All, false);
            if (symbol) {
        ...
        const luaTupleTypeDec = (_c = (_b = this.symbols
            .get(exports.SYMBOL_NAMES.LuaTuple)) === null || _b === void 0 ? void 0 : _b.declarations) === null || _c === void 0 ? void 0 : _c.find(v => byots_1.default.isTypeAliasDeclaration(v));
        if (luaTupleTypeDec) {
    ...
    getPropertyCallMacro(symbol) {
        ...
        if (!macro &&
            ...
            assert_1.assert(false, `Macro ${symbol.parent.name}.${symbol.name}() is not implemented!`);
        }
        return macro;
    }
}
