...
function renderIfStatement(state, node) {
    let result = "";
    result += state.line(`if ${LuauRenderer_1.render(state, node.condition)} then`);
    if (node.statements) {
        result += state.block(() => renderStatements_1.renderStatements(state, node.statements));
    }
    ...
    while (LuauAST_1.default.isNode(currentElseBody)) {
        ...
        result += state.line(`elseif ${LuauRenderer_1.render(state, currentElseBody.condition)} then`);
        result += state.block(() => renderStatements_1.renderStatements(state, statements));
