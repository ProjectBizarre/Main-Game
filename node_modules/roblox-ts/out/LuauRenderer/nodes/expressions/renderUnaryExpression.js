"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.renderUnaryExpression = void 0;
const LuauAST_1 = __importDefault(require("../../../LuauAST"));
const LuauRenderer_1 = require("../..");
const needsParentheses_1 = require("../../util/needsParentheses");
function needsInnerParentheses(node) {
    if ((node.operator === "#" || node.operator === "-") && LuauAST_1.default.isTable(node.expression)) {
        return true;
    }
    return false;
}
function needsSpace(node) {
    if (node.operator === "not") {
        return true;
    }
    if (LuauAST_1.default.isUnaryExpression(node.expression) && node.expression.operator === "-") {
        return true;
    }
    return false;
}
function renderUnaryExpression(state, node) {
    let expStr = LuauRenderer_1.render(state, node.expression);
    let opStr = node.operator;
    if (needsSpace(node)) {
        opStr += " ";
    }
    if (needsInnerParentheses(node)) {
        expStr = `(${expStr})`;
    }
    let result = `${opStr}${expStr}`;
    if (needsParentheses_1.needsParentheses(node)) {
        result = `(${result})`;
    }
    return result;
}
exports.renderUnaryExpression = renderUnaryExpression;
//# sourceMappingURL=renderUnaryExpression.js.map