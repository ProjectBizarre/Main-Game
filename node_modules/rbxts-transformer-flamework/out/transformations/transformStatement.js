...
function transformStatement(state, statement) {
    return diagnosticsUtils_1.catchDiagnostic(statement, function () {
        var _a = __read(state.capture(function () {
            var transformer = TRANSFORMERS.get(statement.kind);
            if (transformer) {
                return transformer(state, statement);
            }
            return typescript_1.default.visitEachChild(statement, function (newNode) { return transformNode_1.transformNode(state, newNode); }, state.context);
        }), 2), node = _a[0], prereqs = _a[1];
        return __spreadArray(__spreadArray([], __read(prereqs)), __read(getNodeList_1.getNodeList(node)));
    });
}
